...
function accountToKindV2(acc: string): string | null {
  const a = String(acc || "");
...
function kindV2ToLegacy(k: string): "goods" | "fixed_asset" | "ccdc" | "service" {
 ...
 */
async function learnLineClassificationsFromApproval(
  supabase: any,
  args: {
    tenantId: string;
    userId: string;
  ...
  // Lấy supplier_tax_id + invoice_lines
  const { data: inv } = await supabase
    .from("invoices")
  ...
  // Tìm tài khoản "dominant" (debit lớn nhất, bỏ 133/111/112/331)
