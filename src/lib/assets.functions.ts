...
export const runMonthlyDepreciation = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    for (const a of assets ?? []) {
      ...
      const openingAccum = Number(a.opening_accumulated ?? 0);
      ...
      let postedAmount = (existing ?? []).reduce((s: number, e: any) => s + Number(e.amount), 0);
      let monthsPosted = openingMonths + (existing ?? []).length;
      ...
      while (cur <= target) {
        ...
        if (monthsPosted >= life) break;
        // Re-derive monthly amount from CURRENT cost on each period.
        // This auto-redistributes after REVALUATION / MAJOR_REPAIR.
        const remainingMonths = life - monthsPosted;
        const remainingNBV = Number(a.cost) - salvage - openingAccum - postedAmount;
        if (remainingNBV <= 0 || remainingMonths <= 0) break;
        const monthlyAmount = remainingNBV / remainingMonths;
        ...
        if (eErr || !entry) { cur = new Date(cur.getFullYear(), cur.getMonth() + 1, 1); continue; }
