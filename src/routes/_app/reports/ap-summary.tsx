...
import { getApSummary, exportApSummaryXlsx } from "@/lib/payables.functions";
...
export const Route = createFileRoute("/_app/reports/ap-summary")({
...
function ApSummaryPage() {
  ...
  const apFn = useServerFn(getApSummary);
  const exportFn = useServerFn(exportApSummaryXlsx);

  const ap = useQuery({
    queryKey: ["ap-summary", from, to, dims],
    queryFn: () => apFn({ data: { from, to, dims } }),
  ...
  const rows = useMemo(() => {
    ...
    return all.filter((r) => {
      ...
      if (!q) return true;
      return (
        norm(r.supplier_name).includes(q) ||
        norm(r.supplier_code ?? "").includes(q)
      );
    });
  }, [ap.data, search, hideZero]);
  ...
  async function handleExport() {
    try {
      toast.loading("Đang xuất Excel...", { id: "xlsx-ap" });
      const res = await exportFn({ data: { from, to, dims } });
      const link = document.createElement("a");
      link.href = `data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,${res.base64}`;
      link.download = res.filename;
      link.click();
      toast.success("Đã xuất file", { id: "xlsx-ap" });
    } catch (e: any) {
      toast.error(e.message ?? "Xuất file thất bại", { id: "xlsx-ap" });
    }
  }
  ...
  return (
    <div className="p-8 print:p-0">
      <div className="flex items-center justify-between print:hidden">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Tổng hợp số dư đầu kỳ, phát sinh và số dư cuối kỳ theo nhà cung cấp (TK 331)
          </p>
        </div>
      ...
      <div className="mt-4 print:mt-0">
        <PrintHeader
          profile={profile}
          title="BẢNG TỔNG HỢP CÔNG NỢ PHẢI TRẢ"
          subtitle={`Tài khoản 331 · Kỳ từ ${from} đến ${to}`}
        />
        <ReportCard
          title="Công nợ phải trả theo nhà cung cấp"
          subtitle={`${rows.length} nhà cung cấp`}
        >
          {ap.isLoading ? (
            <Loading />
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-muted/40 text-xs uppercase">
                  <tr>
                    <th rowSpan={2} className="px-3 py-2 text-left align-bottom">Mã NCC</th>
                    <th rowSpan={2} className="px-3 py-2 text-left align-bottom">Tên nhà cung cấp</th>
                ...
                <tbody>
                  {rows.map((r, i) => (
                    <tr key={(r.supplier_id ?? r.supplier_name) + i} className="border-t border-border">
                      <td className="px-3 py-2 font-mono text-xs">{r.supplier_code ?? ""}</td>
