...
import { salesBySalesperson } from "@/lib/sales-reports.functions";
import { ReportShell } from "@/components/reports/report-shell";
import { ReportTable, type Col } from "@/components/reports/report-table";
import { downloadCsv } from "@/lib/csv-export";

export const Route = createFileRoute("/_app/sales-dashboard/reports/by-salesperson")({
  component: Page,
});
...
type Row = Awaited<ReturnType<typeof salesBySalesperson>>["rows"][number];

function Page() {
  const [from, setFrom] = useState(firstOfMonth());
  const [to, setTo] = useState(todayStr());
  const fn = useServerFn(salesBySalesperson);
  const q = useQuery({
    queryKey: ["sales-by-salesperson", from, to],
  ...
  const handleExport = () => {
    if (!q.data) return;
    downloadCsv(
      `ban-hang-theo-nhan-vien_${from}_${to}.csv`,
  ...
  return (
    <ReportShell
      title="Bảng tổng hợp bán hàng theo nhân viên"
      subtitle="Nhân viên = người phụ trách đơn đặt hàng nguồn của hoá đơn"
      from={from}
      to={to}
