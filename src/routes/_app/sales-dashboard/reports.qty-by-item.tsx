...
import { salesQtyByItem } from "@/lib/sales-reports.functions";
import { ReportShell } from "@/components/reports/report-shell";
import { ReportTable, type Col } from "@/components/reports/report-table";
import { downloadCsv } from "@/lib/csv-export";

export const Route = createFileRoute("/_app/sales-dashboard/reports/qty-by-item")({
  component: Page,
});
...
type Row = Awaited<ReturnType<typeof salesQtyByItem>>["rows"][number];

function Page() {
  const [from, setFrom] = useState(firstOfMonth());
  const [to, setTo] = useState(todayStr());
  const fn = useServerFn(salesQtyByItem);
  const q = useQuery({
    queryKey: ["sales-qty-by-item", from, to],
  ...
  const handleExport = () => {
    if (!q.data) return;
    downloadCsv(
      `ban-hang-theo-so-luong_${from}_${to}.csv`,
  ...
  return (
    <ReportShell
      title="Bảng tổng hợp bán hàng theo số lượng sản phẩm"
      from={from}
      to={to}
