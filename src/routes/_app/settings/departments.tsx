...
import { Plus, Pencil, Trash2, Users, ChevronLeft } from "lucide-react";

export const Route = createFileRoute("/_app/settings/departments")({ component: DepartmentsPage });
  ...
  return (
    <div className="p-6 space-y-4 max-w-5xl">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold flex items-center gap-2"><Users className="h-6 w-6" />Phòng ban</h1>
          <p className="text-sm text-muted-foreground">Cấu trúc phòng ban theo chi nhánh.</p>
        </div>
        <DeptDialog rows={rows} branches={(branches as any[]) ?? []} />
      </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-muted/40 text-xs uppercase">
            <tr>
              ...
              <th className="px-4 py-2 text-left">Phòng ban cha</th>
            ...
            {rows.map((r) => (
              <tr key={r.id} className="border-t border-border hover:bg-muted/30">
                ...
                <td className="px-4 py-2 text-right space-x-1">
                  <DeptDialog row={r} rows={rows} branches={(branches as any[]) ?? []} />
                  <DeleteRow id={r.id} name={r.name} />
                </td>
              </tr>
            ))}
            {rows.length === 0 && <tr><td colSpan={7} className="px-4 py-12 text-center text-muted-foreground">Chưa có phòng ban nào</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
...
function DeptDialog({ row, rows, branches }: { row?: any; rows: any[]; branches: any[] }) {
  const up = useServerFn(upsertDepartment);
  const qc = useQueryClient();
  const [open, setOpen] = React.useState(false);
  const [form, setForm] = React.useState(() => ({
    code: row?.code ?? "", name: row?.name ?? "",
    parent_id: row?.parent_id ?? null, branch_id: row?.branch_id ?? null,
    manager: row?.manager ?? "", is_active: row?.is_active ?? true,
  }));
  const m = useMutation({
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        {row ? <Button variant="ghost" size="icon"><Pencil className="h-4 w-4" /></Button> : <Button><Plus className="mr-2 h-4 w-4" />Thêm phòng ban</Button>}
      </DialogTrigger>
      <DialogContent>
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1">
              <Label className="text-xs">Phòng ban cha</Label>
          ...
          <div className="flex items-center gap-2"><Switch checked={form.is_active} onCheckedChange={(v) => setForm({ ...form, is_active: v })} id="d-active" /><Label htmlFor="d-active" className="text-sm">Hoạt động</Label></div>
        </div>
        <DialogFooter>
...
function DeleteRow({ id, name }: { id: string; name: string }) {
  ...
  return (
    <AlertDialog>
      ...
      <AlertDialogContent>
        <AlertDialogHeader><AlertDialogTitle>Xoá phòng ban "{name}"?</AlertDialogTitle><AlertDialogDescription>Hành động này không thể hoàn tác.</AlertDialogDescription></AlertDialogHeader>
