...
import { Info, Settings as SettingsIcon, ArrowRight, ArrowRightLeft, Activity } from "lucide-react";
import { sampleAgents, sampleOrchestrationFlow, getAgentById } from "@/data/sampleAgents";
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-lg border bg-card p-5">
        ...
        <OrchestrationFlowView onNodeClick={scrollToAgent} />
      ...
      <section className="rounded-lg border bg-card p-5">
        ...
        <ul className="divide-y">
          {allActivities.map((act) => {
            const agent = getAgentById(act.agent_id)!;
            return (
              <li
                key={`${act.agent_id}-${act.id}`}
...
function OrchestrationFlowView({ onNodeClick }: { onNodeClick: (id: string) => void }) {
  const steps = sampleOrchestrationFlow.steps;

  // Group by order, then handle parallel groups
      ...
      {grouped.map((group, gi) => (
        <div key={gi} className="flex items-center gap-2">
          <div className="flex flex-col gap-1">
            {group.map((step) => {
              const agent = getAgentById(step.agent_id)!;
              return (
                <button
                  key={step.agent_id}
